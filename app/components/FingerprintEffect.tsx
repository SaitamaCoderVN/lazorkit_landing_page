...
export default function FingerprintEffect() {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < POOL_SIZE; i++) {
      ...
      el.innerHTML = FP;
      el.style.cssText =
        "position:fixed;pointer-events:none;z-index:9999;opacity:0;transform:scale(1);will-change:transform,opacity;";
      container.appendChild(el);
      pool.push(el);
    }
    poolRef.current = pool;
    return () => pool.forEach((el) => el.remove());
  }, []);
  ...
  const spawn = useCallback((x: number, y: number) => {
    ...
    const el = pool[indexRef.current % POOL_SIZE];
    indexRef.current++;

    // Stamp: appear instantly
    ...
    // Press down
    el.style.transition = "transform 0.15s cubic-bezier(0.2,0,0.5,1)";
    el.style.transform = "scale(0.92)";

    // Bounce back
    setTimeout(() => {
      el.style.transition = "transform 0.2s cubic-bezier(0,0.5,0.3,1.3)";
